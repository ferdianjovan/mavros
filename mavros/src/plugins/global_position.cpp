...
public:
    ...
    void initialize(UAS &uas_) override
    {
        ...
        raw_unfix_pub = gp_nh.advertise<sensor_msgs::NavSatFix>("raw/unfix", 10);
...
private:
    ...
    void handle_gps_raw_int(const mavlink::mavlink_message_t *msg, mavlink::common::msg::GPS_RAW_INT &raw_gps)
    {
        ...
        auto unfix = boost::make_shared<sensor_msgs::NavSatFix>();

        fix->header = m_uas->synchronized_header(child_frame_id, raw_gps.time_usec);
        unfix->header = m_uas->synchronized_header(child_frame_id, raw_gps.time_usec);

        fix->status.service = sensor_msgs::NavSatStatus::SERVICE_GPS;
        unfix->status.service = sensor_msgs::NavSatStatus::SERVICE_GPS;
        if (raw_gps.fix_type > 2) {
            fix->status.status = sensor_msgs::NavSatStatus::STATUS_FIX;
            unfix->status.status = sensor_msgs::NavSatStatus::STATUS_FIX;
        }
        else {
            ROS_WARN_THROTTLE_NAMED(30, "global_position", "GP: No GPS fix");
            fix->status.status = sensor_msgs::NavSatStatus::STATUS_NO_FIX;
            unfix->status.status = sensor_msgs::NavSatStatus::STATUS_NO_FIX;
        }

        fill_lla(raw_gps, fix);
        fill_lla(raw_gps, unfix);
        ...
        // ftf::EigenMapCovariance3d gps_cov(unfix->position_covariance.data());
        ...
        if (msg->magic == MAVLINK_STX &&
            ...
            unfix->position_covariance_type = sensor_msgs::NavSatFix::COVARIANCE_TYPE_DIAGONAL_KNOWN;
        }
        // With mavlink v1.0 approximate accuracies by DOP
            ...
            unfix->position_covariance_type = sensor_msgs::NavSatFix::COVARIANCE_TYPE_APPROXIMATED;
        }
        else {
            fill_unknown_cov(fix);
            fill_unknown_cov(unfix);
        }

        // store & publish
        m_uas->update_gps_fix_epts(fix, eph, epv, raw_gps.fix_type, raw_gps.satellites_visible);
        unfix->altitude = raw_gps.alt / 1E3;
        raw_unfix_pub.publish(unfix);
